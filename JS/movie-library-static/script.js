...
data.forEach(function (mv)
{
    const movie = document.createElement('div')
    movie.classList.add('movie')
    const img = document.createElement('img');
    const poster=mv.Poster.replace('http','https')
    img.setAttribute('src',poster)
    img.alt = mv.Title;
    const title = document.createElement('h2')
    title.textContent = mv.Title;
    const Director = document.createElement('p')
    Director.textContent = "Director:" + mv.Director;
    console.log(Director)
    const year = document.createElement('p');
    year.textContent = "YEAR:" + mv.Year;
    const btn = document.createElement('button')
    btn.addEventListener('click', incr)
    btn.textContent="addtocart"
    movie.append(img, title, Director, year,btn)
